<template>
  <div>
    ...
    <group :title="$t('Application Settings')">
      ...
      <cell :title="$t('Log Settings')" is-link @click.native="loglevelSelection = !loglevelSelection"></cell>
      <cell :title="$t('Battery Settings')" is-link>
      </cell>
    </group>

    <div v-transfer-dom>
           <popup v-model="loglevelSelection" height="33%">
                      <div class="popup1">
    ...
    @on-click-right="loglevelSelection = false">
    ...
    :left-text="$t('Back')"
    :right-text="$t('')"
    :title="$t('Vibration Settings')"
    @on-click-left="showvibrationsettings = false"
...
export default {
  ...
  data () {
    return {
      radio001: [ 'VERBOSE', 'DEBUG', 'INFO', 'WARN', 'ERROR' ],
      loglevelSelection: false,
      showvibrationsettings: false,
      checklist001: true,
      checklist002: true,
      confirmDelete: false,
      selectedContact: {},
